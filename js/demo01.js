$(function(){
	...
	$.get("php/getGoodsList.php",function(data){
		...
		let	str='<li><img src="img/'+res[i].goodsImg+'" class="'+res[i].goodsId+'"/></li>';
			$(".container-top .container0 .containerTop").append(str);
		}
		xiaoguo0();
		$(".container-top").mouseenter(function(){
			$(this).children("a").css({
				"display":"block"
			})
			$(this).children(".container0").css({
				"background":"#ADFF2F"
			})
		})
		...
		var n=parseInt(res.length/4)+1;
//		console.log(n);
		var m=0;
		$(".container-top .next").click(function(){
			m++;
			if(m==n){
				m=0;
			}
...
//					//-------------购物车中数量加减------------------
	...
	function xiaoguo0(){
		$.get("php/getGoodsList.php",function(data){
			...
			var x;
			var a;
			$(".container-top img").click(function(){
				$(".container").text("");
				$("#box #banner").css({
					"display":"none"
				})
				for(var i=0;i<res.length;i++){
					...
					if($(this).hasClass(a)){
						x=i;
						let containerCenter='<p class="fanhui"><a href="shouye.html">返回&lt;&lt;</a></p><div class="containerLeft"><ul class="containerLeftMin"><li class="active"><img src="img/'+res[x].goodsImg+'" width="50"/></li><li><img src="img/'+res[x].goodsImg+'" width="50"/></li><li><img src="img/'+res[x].goodsImg+'" width="50"/></li></ul><div class="containerLeftMax"><img src="img/'+res[x].goodsImg+'" /></div></div><div class="containerRight"><p><h2>'+res[x].goodsName+'</h2></p><p>果园价:'+res[x].goodsPrice+'</p><p>规格:</p><p>配送至:<select><option>北京</option><option>上海</option><option>天津</option><option>深圳</option><option>西安</option></select></p><p>数量:<input type="button" class="delate" value="-"/><input type="text" class="txt" /><input type="button" class="add" value="+"/></p><p><input type="button" class="buy" value="立即购买"/><input type="button" class="car" value="加入购物车"/></p><div class="containerRightBottom"><p>商品编号:'+res[x].goodsId+'</p><p>备注:'+res[x].goodsDesc+'</p></div></div><div class="containerBottom"><p class="sp"><span class="spjj">商品简介</span><span class="gkpl">顾客评论</span></p><ul class="jianjie"><div class="jianjie2"><img src="img/'+res[x].goodsImg+'" width="500"/><img src="img/'+res[x].goodsImg+'" width="500"/><img src="img/'+res[x].goodsImg+'" width="500"/><img src="img/'+res[x].goodsImg+'" width="500"/></div><div class="gkpl2"></div></ul><textarea class="talking"></textarea></div>';
						$(".container").append(containerCenter);
					//---------------点击添加到购物车-------------
					$(".container .containerRight .car").click(function(){
						let htmlStr='<li><img src="img/'+res[x].goodsImg+'" width="100" height="100" class="'+res[x].goodsId+'"/><div class="right"><h2 class="title">'+res[x].goodsName+'</h2><br /><p class="jiage">价格:'+res[x].goodsPrice+'</p><p class="id"></p><input type="button" class="jian" value="-"/><input  type="text" value="1" class="num"/><input type="button" class="jia" value="+"/><a href="###" class="del">删除</a></div></li>';
